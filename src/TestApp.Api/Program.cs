...
var tests = app.MapGroup("/api/v1/tests")
...
tests.MapPatch("/{id}/title", async (Guid id, HttpRequest request, RenameTestRequest r, RenameTestCommandHandler h, CancellationToken ct) =>
{
...
tests.MapPatch("/{id}/settings", async (Guid id, HttpRequest request, TestSettingsRequest r, ChangeTestSettingsCommandHandler h, CancellationToken ct) =>
{
    var version = TestEtags.ResolveRequiredIfMatch(request);
...
tests.MapPost("/{id}/questions", async (Guid id, HttpRequest request, QuestionWriteRequest r, AddQuestionCommandHandler h, CancellationToken ct) =>
{
    var version = TestEtags.ResolveRequiredIfMatch(request);
...
tests.MapPut("/{id}/questions/{questionId}", async (Guid id, Guid questionId, HttpRequest request, QuestionUpdateRequest r, UpdateQuestionCommandHandler h, CancellationToken ct) =>
{
    var version = TestEtags.ResolveRequiredIfMatch(request);
...
tests.MapDelete("/{id}/questions/{questionId}", async (Guid id, Guid questionId, HttpRequest request, RemoveQuestionCommandHandler h, CancellationToken ct) =>
{
    var version = TestEtags.ResolveRequiredIfMatch(request);
...
tests.MapPatch("/{id}/questions/{questionId}/order", async (Guid id, Guid questionId, HttpRequest request, OrderRequest r, ReorderQuestionCommandHandler h, CancellationToken ct) =>
{
    var version = TestEtags.ResolveRequiredIfMatch(request);
...
tests.MapPost("/{id}/questions/{questionId}/options", async (Guid id, Guid questionId, HttpRequest request, AnswerOptionWriteRequest r, AddAnswerOptionCommandHandler h, CancellationToken ct) =>
{
    var version = TestEtags.ResolveRequiredIfMatch(request);
...
tests.MapPut("/{id}/questions/{questionId}/options/{optionId}", async (Guid id, Guid questionId, Guid optionId, HttpRequest request, AnswerOptionUpdateRequest r, UpdateAnswerOptionCommandHandler h, CancellationToken ct) =>
{
    var version = TestEtags.ResolveRequiredIfMatch(request);
...
tests.MapDelete("/{id}/questions/{questionId}/options/{optionId}", async (Guid id, Guid questionId, Guid optionId, HttpRequest request, RemoveAnswerOptionCommandHandler h, CancellationToken ct) =>
{
    var version = TestEtags.ResolveRequiredIfMatch(request);
...
tests.MapPatch("/{id}/questions/{questionId}/options/{optionId}/order", async (Guid id, Guid questionId, Guid optionId, HttpRequest request, OrderRequest r, ReorderAnswerOptionCommandHandler h, CancellationToken ct) =>
{
    var version = TestEtags.ResolveRequiredIfMatch(request);
...
tests.MapPost("/{id}/publish", async (Guid id, HttpRequest request, PublishRequest r, PublishTestCommandHandler h, CancellationToken ct) =>
{
    var version = TestEtags.ResolveRequiredIfMatch(request);
...
tests.MapPost("/{id}/archive", async (Guid id, HttpRequest request, ArchiveTestCommandHandler h, CancellationToken ct) =>
{
    var version = TestEtags.ResolveRequiredIfMatch(request);
    return version.IsSuccess ? ToHttp(await h.Handle(new ArchiveTestCommand(new TestId(id), version.Value), ct)) : version.Error!;
}).RequireAuthorization(Permissions.TestsWrite);
tests.MapGet("/{id}/editor", async (Guid id, HttpResponse response, GetTestEditorViewQueryHandler h, CancellationToken ct) =>
{
...
tests.MapGet("/{id}/revisions", async (Guid id, GetTestRevisionsQueryHandler h, CancellationToken ct) =>
...
var assignments = app.MapGroup("/api/v1/assignments")
    .RequireAuthorization()
    .AddEndpointFilter<RequestValidationFilter>();
...
assignments.MapGet("/{id}", async (Guid id, GetAdminAssignmentQueryHandler h, CancellationToken ct) =>
...
assignments.MapGet("/{id}/attempts", async (Guid id, AttemptStatus? status, AttemptOutcome? outcome, int? page, int? pageSize, GetAdminAssignmentAttemptsQueryHandler h, CancellationToken ct) =>
...
assignments.MapPatch("/{id}/window", async (Guid id, AssignmentWindowRequest r, ChangeAssignmentWindowCommandHandler h, CancellationToken ct) => ToHttp(await h.Handle(new ChangeAssignmentWindowCommand(new TestAssignmentId(id), r.AvailableFrom, r.AvailableUntil), ct))).RequireAuthorization(Permissions.TestsAssign);
assignments.MapPatch("/{id}/attempt-limit", async (Guid id, AttemptLimitRequest r, ChangeAssignmentAttemptLimitCommandHandler h, CancellationToken ct) => ToHttp(await h.Handle(new ChangeAssignmentAttemptLimitCommand(new TestAssignmentId(id), r.AttemptLimit), ct))).RequireAuthorization(Permissions.TestsAssign);
assignments.MapPost("/{id}/cancel", async (Guid id, CancelAssignmentRequest r, CancelAssignmentCommandHandler h, CancellationToken ct) => ToHttp(await h.Handle(new CancelAssignmentCommand(new TestAssignmentId(id), r.Reason), ct))).RequireAuthorization(Permissions.TestsAssign);
assignments.MapPost("/{id}/attempts", async (Guid id, HttpRequest request, StartAttemptRequest r, StartAttemptCommandHandler h, CancellationToken ct) =>
{
    var key = IdempotencyKeyResolver.Resolve(request, r.IdempotencyKey);
    return key.IsSuccess
...
app.MapGet("/api/v1/me/assignments", async (int? page, int? pageSize, AssignmentStatus? status, GetMyAssignmentsQueryHandler h, CancellationToken ct) =>
    Results.Ok(await h.Handle(new GetMyAssignmentsQuery(page ?? 1, pageSize ?? 20, status), ct)))
    .RequireAuthorization()
    .AddEndpointFilter<RequestValidationFilter>();
app.MapGet("/api/v1/me/attempts", async (int? page, int? pageSize, AttemptStatus? status, GetMyAttemptsQueryHandler h, CancellationToken ct) =>
    Results.Ok(await h.Handle(new GetMyAttemptsQuery(page ?? 1, pageSize ?? 20, status), ct)))
    .RequireAuthorization()
    .AddEndpointFilter<RequestValidationFilter>();
app.MapGet("/api/v1/results", async (Guid? testId, Guid? revisionId, AttemptOutcome? outcome, int? page, int? pageSize, GetReviewerResultsQueryHandler h, CancellationToken ct) =>
    Results.Ok(await h.Handle(new GetReviewerResultsQuery(testId is null ? null : new TestId(testId.Value), revisionId is null ? null : new PublishedTestRevisionId(revisionId.Value), outcome, page ?? 1, pageSize ?? 20), ct)))
    .RequireAuthorization(Permissions.ResultsReview)
    .RequireRateLimiting(RatePolicies.PrivilegedRead)
    .AddEndpointFilter<RequestValidationFilter>();
app.MapGet("/api/v1/results/{attemptId}", async (Guid attemptId, GetReviewerAttemptResultQueryHandler h, CancellationToken ct) =>
...
var attempts = app.MapGroup("/api/v1/attempts")
    .RequireAuthorization()
    .AddEndpointFilter<RequestValidationFilter>();
attempts.MapPut("/{id}/answers/{questionId}", async (Guid id, Guid questionId, AnswerQuestionRequest r, AnswerQuestionCommandHandler h, CancellationToken ct) => ToHttp(await h.Handle(new AnswerQuestionCommand(new TestAttemptId(id), new QuestionId(questionId), r.OptionIds.Select(x => new AnswerOptionId(x)).ToArray()), ct))).RequireRateLimiting(RatePolicies.StudentWrite);
attempts.MapDelete("/{id}/answers/{questionId}", async (Guid id, Guid questionId, ClearAnswerCommandHandler h, CancellationToken ct) => ToHttp(await h.Handle(new ClearAnswerCommand(new TestAttemptId(id), new QuestionId(questionId)), ct))).RequireRateLimiting(RatePolicies.StudentWrite);
attempts.MapPost("/{id}/submit", async (Guid id, HttpRequest request, SubmitAttemptRequest r, SubmitAttemptCommandHandler h, CancellationToken ct) =>
{
    var key = IdempotencyKeyResolver.Resolve(request, r.IdempotencyKey);
    return key.IsSuccess
...
attempts.MapPost("/{id}/timeout", async (Guid id, TimeoutAttemptCommandHandler h, CancellationToken ct) => ToHttp(await h.Handle(new TimeoutAttemptCommand(new TestAttemptId(id)), ct))).RequireAuthorization(Permissions.TestsAssign);
attempts.MapGet("/{id}", async (Guid id, GetAttemptQueryHandler h, CancellationToken ct) => await h.Handle(new GetAttemptQuery(new TestAttemptId(id)), ct) is { } value ? Results.Ok(value) : Results.NotFound());
attempts.MapGet("/{id}/result", async (Guid id, GetAttemptResultQueryHandler h, CancellationToken ct) => await h.Handle(new GetAttemptResultQuery(new TestAttemptId(id)), ct) is { } value ? Results.Ok(value) : Results.NotFound());
