...
## ADR-031 — Staging-контур как одна VM с Compose, а не managed-сервисы (`#16`)

**Статус:** принято.

**Решение:** контур гейта 1.0 разворачивается на одной виртуальной машине через `compose.staging.yaml`: PostgreSQL, RabbitMQ, Keycloak, API, OTel Collector, Prometheus, Grafana и Caddy как ingress. Managed PostgreSQL/RabbitMQ не используются.

**Почему не managed, хотя обычно советуют обратное.** `#16` формулировал выбор как «для 1.0 достаточно первого варианта». Достаточность — не главный аргумент; для этого гейта VM подходит *лучше*. Посмотрите, что контур обязан обслужить: из четырёх блокируемых issue три требуют намеренно ломать инфраструктуру.

- `#19` — alert drill: остановить API и базу, остановить RabbitMQ, заблокировать `OverdueAttemptProcessor` и измерить time-to-alert.
- `#18` — restore drill: снять дамп и восстановить его **вашими** скриптами `postgresql-backup.sh`/`postgresql-restore-verify.sh` с измеренным RTO.
- `#23` — репетиция релиза: развернуть, сломать, откатиться на предыдущий digest.

На managed-сервисах брокер не останавливается по вашему усмотрению, а восстановление превращается в проверку снапшотов провайдера вместо проверки ваших процедур. То есть драйв перестаёт измерять то, ради чего затевался. Managed оправдан, когда staging обязан повторять эксплуатационную модель production; у проекта эта модель ещё не выбрана, а гейт проверяет процедуры.

**Что отличает контур от локального стенда** — и почему `compose.yaml` нельзя просто запустить на VM:

- профиль `Staging`, а не `Development`: HSTS и security headers включены, OpenAPI закрыт ролью администратора, `Keycloak:RequireHttpsMetadata=true`;
- ни одного отладочного креденшела: все секреты приходят из `.env` на самой машине, шаблон — `.env.staging.example`, сам файл под `.gitignore`;
- отдельный realm `testapp-staging` **без пользователей**: фикстуры заводит оператор вручную, поэтому в репозитории не лежит ни одного действующего креденшела контура, а клиент `testapp-api` — confidential;
- образ задаётся digest'ом, а не тегом, и тот же digest уезжает в `TESTAPP_IMAGE_DIGEST`, поэтому `GET /api/v1/operations/version` отвечает, что именно запущено;
- наружу опубликован ровно один порт — ingress; PostgreSQL, RabbitMQ, Prometheus и Grafana доступны только внутри сети.

**Ловушка, из-за которой контур может дать ложные измерения.** Лимит частоты ключуется по claim `sub`, а для анонимных запросов — по `RemoteIpAddress` (`RateLimitPartitioning.cs`), при этом `ForwardedHeaders` очищает список доверенных прокси и заполняет его только из конфигурации. Если поставить ingress и не объявить `ReverseProxy:KnownNetworks`, весь анонимный трафик схлопнется в одну партицию по адресу прокси: нагрузочный прогон `#20` упрётся в `429`, не имеющие отношения к ёмкости, эти цифры попадут в `PERFORMANCE.md` как baseline 1.0, а по ним откалибруются пороги алертов `#19`. Обе ошибки будут выглядеть как настоящие измерения. Поэтому доверенная сеть задана в `compose.staging.yaml` обязательной переменной, а не оставлена на усмотрение.

**Второе следствие того же рода:** k6 нельзя запускать на самой VM — генератор нагрузки будет конкурировать за CPU с системой под тестом, и `p95` измерит борьбу за планировщик, а не ёмкость.

**Условие пересмотра:** появление production-окружения с выбранной эксплуатационной моделью. Тогда staging имеет смысл привести к ней, даже ценой усложнения drill'ов.

## ADR-032 — `Microsoft.OpenApi` остаётся на 2.x до конца жизни ASP.NET Core 10 (`#24`)

**Статус:** принято. *(ADR-031 зарезервирован за выбором носителя staging, issue #16.)*

**Решение:** `Microsoft.AspNetCore.OpenApi` поднимается до `10.0.11`, `Microsoft.OpenApi` остаётся на `2.11.0`. Переход на `3.x` до 1.0 не выполняется и не планируется в рамках 10.x.

**Почему это отдельное решение, а не рутинное обновление:** issue #22 замораживает публичный контракт v1 снимком `/openapi/v1.json` и защищает его diff-тестом. `Microsoft.OpenApi` — прямая зависимость `TestApp.Api`, на её типах написаны три трансформера документа, и именно она сериализует ответ `MapOpenApi`. Снять снимок под одной версией и обновиться под другой — значит получить сломанный diff-тест, в котором нельзя отличить изменение контракта от изменения сериализатора. Поэтому версия фиксируется до заморозки.

**Что показала проверка (`3.10.0`):** обновление не проходит сборку, и блокирует его не наш код.

1. `src/TestApp.Api/OpenApi/ApiContractOperationTransformer.cs:128` — `CS0266`, `Dictionary<string, OpenApiMediaType>` больше не приводится к `IDictionary<string, IOpenApiMediaType>`. Это наш код, чинится.
2. `OpenApiXmlCommentSupport.generated.cs:399` и `:461` — `CS0200`, `IOpenApiMediaType.Example` в `3.x` только для чтения. **Этот файл генерирует `Microsoft.AspNetCore.OpenApi.SourceGenerators`**, входящий в состав framework-пакета и скомпилированный против `2.x`. Починить его нельзя, не отключив генератор XML-комментариев или не сменив версию ASP.NET Core.

Решающее подтверждение пришло из метаданных самого пакета: `Microsoft.AspNetCore.OpenApi 10.0.10` объявлял зависимость `Microsoft.OpenApi >= 2.0.0` без верхней границы, а `10.0.11` объявляет `[2.7.5, 3.0.0)` — Microsoft явно закрыла `3.x` для ASP.NET Core 10. Значит это не вопрос «совместимо ли», а вопрос поддерживаемого диапазона, и обновление до `3.x` требует смены major-версии ASP.NET Core.

**Почему не `2.12.0`, хотя он внутри разрешённого диапазона:** `2.12.0` помечает `OpenApiSchema.Example` устаревшим в пользу `Examples`, что даёт шесть `CS0618` в `ApiExampleSchemaTransformer.cs` при стандарте проекта «ноль предупреждений». Миграция на `Examples` меняет сам документ — `example: {…}` шести request-схем превращается в `examples: [{…}]` — и требует правки `OpenApiContractTests` (строки 86 и 90 проверяют наличие `example`). Править тесты, стерегущие контракт, непосредственно перед его заморозкой — ровно то, чего делать нельзя. Изменение отложено до появления снимка v1, тогда diff-тест покажет, что именно сдвинулось.

**Измерено, а не предположено:** документ снят с живого хоста на трёх сочетаниях версий — `2.11.0`+`10.0.10`, `2.12.0`+`10.0.11`, `2.11.0`+`10.0.11`. Во всех трёх он побайтово идентичен: 400 072 байта, `"openapi": "3.1.1"`. То есть принятое обновление framework-пакета контракт-нейтрально. Способ снятия: `WebApplicationFactory<Program>` с `Database:ApplyMigrationsOnStartup=false` и заведомо недоступной строкой подключения — эндпоинт OpenAPI не обращается к базе, поэтому PostgreSQL для замера не нужен.

**Условие пересмотра:** выпуск `Microsoft.AspNetCore.OpenApi`, объявляющий диапазон, допускающий `3.x`. До этого момента явный `PackageReference` на `2.11.0` избыточен по отношению к диапазону framework-пакета, но оставлен намеренно: он делает версию сериализатора контракта видимой в diff, а не выведенной resolver'ом.

**Что осталось за рамками:** `xunit.v3` `3.2.1` → `4.0.0` — тоже major и тоже требует review по ADR-008, но в образ не попадает и на контракт не влияет; выносится за пределы гейта 1.0.

## ADR-033 — Каталог интеграционных событий 1.0 пуст намеренно (`#21`, гейт §7 пункт 9)

**Статус:** принято.

**Решение:** на 1.0 наружу не публикуется ни одного интеграционного события. Транспорт Outbox/RabbitMQ остаётся полностью реализованным и протестированным, но ни один production-тип не реализует `IIntegrationEvent`. Каталог `docs/EVENTS_AND_OUTBOX.md` §13 остаётся списком *предлагаемых* контрактов, а не действующих.

**Почему это решение, а не невыполненная работа.** Гейт §7 пункт 9 требует «явно задокументированный каталог интеграционных событий», и прочитать это можно двумя способами. Первый — спроектировать `TestRevisionPublishedV1`/`TestAssignedV1`/`AttemptCompletedV1` и выпустить их с 1.0. Второй — явно зафиксировать, что каталог пуст. Выбран второй, по трём причинам:

1. **Потребителя нет.** Спроектированный без потребителя контракт — это замороженная догадка. Схема интеграционного события переживает свой первый релиз: поменять её потом можно только через parallel publish или согласованный cutover (§14). Платить эту цену за событие, которое никто не читает, нечем оправдать.
2. **Это соответствует уже принятому ADR-012.** Он прямо разрешает транспорту быть готовым раньше каталога и запрещает публиковать внутренние доменные события как внешний контракт. Пустой каталог — следствие этого решения, а не отступление от него.
3. **Это соответствует линии ADR-022/023/024.** Event Sourcing, Redis и второй broker отклонены по одному и тому же принципу: не вводить сущность до появления измеренной потребности. Событийный контракт — та же категория.

**Что закрывает пункт гейта.** Не наличие событий, а отсутствие неопределённости: состояние каталога зафиксировано в документе и закреплено тестом `IntegrationEventCatalogTests`, который сверяет фактический набор реализаций `IIntegrationEvent` в production-сборках с объявленным каталогом. Сейчас обе стороны пусты.

**Как это ведёт себя при появлении первого события.** Тест падает и называет добавленный тип. Это и есть цель: тип, реализующий `IIntegrationEvent`, немедленно становится публичным контрактом — он попадает в Outbox при первом же `SaveChanges` агрегата, который его поднял, и уходит в RabbitMQ. Молча добавить такое событие нельзя; вместе с ним обязаны обновиться каталог §13, политика версионирования §14 и `CHANGELOG.md`. Проверено красным: временный `ProbeIntegrationEvent` роняет тест с указанием полного имени типа.

**Второй тест удерживает предпосылку.** Смысл пустого каталога держится на том, что `IIntegrationEvent` — отдельный маркер, а не синоним `IDomainEvent`: `AppDbContext` захватывает в Outbox только `OfType<IIntegrationEvent>()`. Если бы маркер начал наследоваться всеми доменными событиями, «каталог пуст» означало бы «наружу уходит всё внутреннее». Поэтому отдельно проверяется, что ни одно доменное событие не является интеграционным.

**Условие пересмотра:** появление первого реального потребителя. Тогда контракт проектируется под него, а не наоборот, и проходит проверку приватности из §13 — внешние identity ID не публикуются без data-classification boundary.

## ADR-034 — Готовность проверяется активной пробой, а не метрикой приложения (`#19`, гейт §7 пункт 12)

**Статус:** принято.

**Решение:** page-правило 1 из `docs/SLO_ALERTS.md` §4 («`/health/ready` отказывает три пробы подряд») реализуется отдельным компонентом — `blackbox_exporter`, который ходит в `/health/ready` по HTTP снаружи. Prometheus скребёт экспортер, а не приложение; правило смотрит на `probe_success{job="testapp-readiness"}`.

**Почему не метрика приложения.** Метрика, которую экспортирует сам процесс, отвечает на вопрос «считает ли приложение себя готовым». Правило спрашивает другое: «отвечает ли приложение». Разница видна ровно в тех отказах, ради которых правило и написано: процесс завис, не принимает соединения, исчерпал пул потоков, его не пускает сеть. Во всех этих случаях метрика просто перестаёт обновляться — и это неотличимо от «отвалился экспорт метрик». Существующее правило `TestAppMetricsPipelineDown` именно это и означает, поэтому его нельзя было выдать за проверку готовности: оно срабатывает и когда приложение здорово, а сломан OTel Collector.

Проба спрашивает то же, что спросил бы балансировщик, и её отказ означает то же, что означал бы вывод реплики из ротации.

**Почему это дешевле, чем кажется.** Эндпоинты `/health/live` и `/health/ready` смонтированы через `IStartupFilter` — до аутентификации и до лимитера частоты. Пробер не нуждается в токене и не расходует бюджет запросов, поэтому проба раз в 15 секунд не влияет ни на измерения ёмкости (`#20`), ни на партиционирование лимитера.

**Порог выражен в пробах, а не в секундах.** `for: 45s` при `scrape_interval: 15s` — это ровно три подряд неуспешные пробы, как сказано в контракте. Формулировка «примерно минута» дала бы правилу другой смысл при изменении интервала.

**Слепота пробера — отдельный алерт.** Если Prometheus не может достучаться до экспортера, page-правило 1 не может сработать в принципе: серии `probe_success` нет, а выражение `== 0` не даёт результата. Молчание в этом случае неотличимо от благополучия, поэтому недоступность пробера объявлена warning-правилом 8 §4. Это не дублирование: одно правило про приложение, второе — про наблюдаемость приложения.

**Чем подтверждено.** Не рассуждением, а прогоном: `probe_success` равен 1 против отвечающей цели и падает в 0, когда цель начинает отвечать 503; правило `TestAppReadinessProbeFailing` при этом доходит до состояния `firing` с меткой `severity: page` и `instance`, называющей проверяемый URL, а не адрес экспортера. Проверка встроена в `scripts/validate-observability-stack.sh`, то есть выполняется в CI на живом стеке, а не только один раз руками. Конфигурация обоих окружений удерживается `PrometheusConfigTests`: job пробера обязан существовать и в локальном, и в контурном конфиге, иначе правило молча теряет источник данных.

**Две пробы, а не одна.** Внутренняя проба (`testapp-readiness`) ходит в `api:8080/health/ready` и отвечает на вопрос «приложение готово». Внешняя (`testapp-ingress`) ходит по публичному адресу через ingress и отвечает на другой: «клиент до него дойдёт». Отказ ingress, просроченный сертификат или сломанная маршрутизация — полная недоступность для пользователя, которую внутренняя проба не заметит, потому что приложение при этом здорово. Обратное тоже верно: внутренний отказ при живом ingress даёт 502, и различить причины по одному сигналу нельзя. Поэтому правила разные и указывают на разные места.

Внешняя проба существует только на контуре: локально ingress не поднимается. Её цель генерируется при подъёме из `.env`, потому что Prometheus не подставляет переменные окружения в `scrape_configs`, а публичное имя у каждой установки своё. Пустой job при этом молчит так же, как исправный, — поэтому отсутствие целей объявлено отдельным правилом `absent(up{job="testapp-ingress"})`.

Публичное имя обязано разрешаться в ingress **изнутри** сети compose: API обращается к Keycloak по тому же адресу, что записан в issuer токена, и подменить его внутренним нельзя — валидация JWT сверяет issuer буквально. Без сетевого алиаса запрос ушёл бы в публичный DNS и вернулся на внешний адрес машины, а hairpin NAT не работает ни на большинстве роутеров, ни в WSL2. Контур выглядел бы поднятым и не пускал бы никого.

**Что этим НЕ закрыто.** Реализованы правила и их источники. Drill — то есть подтверждённая доставка page-алерта до дежурного с измеренным time-to-alert — остаётся открытым пунктом `#19`. Внешняя проба в CI не проверяется: ingress поднимается только на контуре.

**Условие пересмотра:** появление нескольких реплик за балансировщиком. Тогда цель пробы — не один адрес, а список реплик (service discovery), и формулировка «на всех обслуживающих репликах» из §4 станет буквальной.

## ADR-035 — Контракт v1 заморожен окончательно до прохождения фронтендом happy path (`#22`, гейт §7 пункт 1)

**Статус:** принято.

**Решение:** заморозка контракта v1 объявляется окончательной. Ломающее изменение из списка `docs/API.md` §1 с этого момента требует v2. Механически заморозка удерживается двумя проверками: `OpenApiSnapshotTests` делает видимым любое изменение, `OpenApiFrozenSurfaceTests` запрещает исчезновение того, что в контракте уже было.

**Принятый риск.** Issue #22 предлагал компромисс: черновая заморозка сейчас, окончательная — после того как фронтенд роли `test-admin` пройдёт happy path всех трёх ролей. Окончательная заморозка объявляется до этого, сознательно. Последствие названо прямо: если фронтенду понадобится изменение контракта, у него остаётся три исхода — дополнение (разрешено внутри v1), v2, либо задокументированное ломающее изменение с подъёмом мажорной версии. Тихо «поправить контракт под фронт» больше нельзя: обе проверки красные.

Риск принят потому, что цена альтернативы выше. Черновая заморозка — это состояние, в котором контракт формально заморожен, а фактически меняется; в таком режиме 1.0 выпускать нельзя, потому что клиенту нечего пообещать. Отложить же 1.0 до готовности фронтенда означает отложить и всё, что от него не зависит.

**Почему двух проверок, а не одной.** Снимок сравнивает документ целиком и падает на любом различии — это делает изменение видимым. Но обновляется он одной командой, и в его diff «добавили поле» и «удалили поле» выглядят одинаково, тогда как ломает клиентов только второе. Поверхность проверяется на включение: дополнения проходят молча, исчезновения падают. У файла поверхности нет автоматической регенерации намеренно — команда «обнови и закоммить» превратила бы заморозку в описание того, что получилось.

**Что заморожено с известной неточностью.** Генератор перечисляет в `required` все поля record'а, включая nullable. `AssignRequest.userId` и `AssignRequest.groupId` объявлены обязательными, хотя ровно один из них обязан быть `null`; `attemptLimit` и `availableUntil` необязательны по смыслу. Документ строже реализации, и это не мешает заморозке: ослабление `required` — не ломающее изменение, клиент, написанный по строгому документу, уже отправляет эти ключи. Приведение документа к смыслу возможно внутри v1 и делается отдельным изменением, а не в момент заморозки.

**403 против 404 на чужом ресурсе.** Чтение чужого теста отвечает `404`, а попытка изменить — `403`. Разница наблюдаема и в теории позволяет установить существование чужого теста по прямой ссылке. Оставлено как есть осознанно: идентификаторы — GUID v7 с 74 битами случайности, перебор нереализуем, а `403` информативнее для легитимного клиента, который получил ссылку от коллеги. Решение зафиксировано здесь, потому что после заморозки смена кода ответа для того же условия — ломающее изменение.

**Условие пересмотра:** появление второго независимого потребителя API. Пока потребитель один и разрабатывается вместе с сервером, v2 — дорогая церемония; когда их станет двое, дешевле окажется именно она.
