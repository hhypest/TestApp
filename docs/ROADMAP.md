...
1. заморозка публичного контракта API v1 (**ЧАСТИЧНО:** снимок `docs/openapi/v1.json` и diff-тест `OpenApiSnapshotTests` введены, политика совместимости — `docs/API.md` §1; заморозка объявлена черновой до прохождения фронтом happy path трёх ролей — issue #22);
2. ноль открытых дефектов P0 по безопасности и изоляции данных (**ЧАСТИЧНО:** чеклист с указанием проверки для каждого утверждения — `docs/SECURITY.md` §18.1; открытыми остаются две строки, требующие staging);
