...
export KEYCLOAK_REALM=testapp-staging KEYCLOAK_CLIENT_SECRET=...
export KEYCLOAK_ADMIN_USERNAME=... KEYCLOAK_ADMIN_PASSWORD=...
export AUTHOR_USERNAME=... AUTHOR_PASSWORD=... ADMIN_USERNAME=... ADMIN_PASSWORD=...
...
#### Две переприсадки: `ForwardLimit` обязан стать двойкой

Это главное. Цепочка запроса — клиент → роутер → Caddy → API. Caddy дописывает в `X-Forwarded-For` адрес того, от кого получил запрос, то есть роутера. При `ForwardLimit=1` приложение снимет ровно одну запись и решит, что клиент — это `192.168.3.1`.

Последствие ровно то же, о котором предупреждает §16.0.1, но причина другая: **весь анонимный трафик снова схлопнется в одну партицию лимитера**, и цифры `#20` будут получены под `429`, не связанными с ёмкостью.

```dotenv
TESTAPP_FORWARD_LIMIT=2
TESTAPP_INTERNAL_CIDR=<CIDR сети compose из шага 7>
TESTAPP_UPSTREAM_PROXY_CIDR=192.168.3.1/32
```

Роутер объявляется **маской `/32`**, то есть ровно одним адресом, а не сетью `192.168.3.0/24`: доверие целой домашней сети означало бы, что любой её хост может подделать `X-Forwarded-For` и обойти лимит частоты. Значение по умолчанию — `127.0.0.1/32`, то есть «второй переприсадки нет»: пустая строка уронила бы разбор CIDR при старте, а loopback безвреден.

Проверяется тем же запросом, что и в §16.0.1, — в аудите должны быть адреса клиентов, а не `192.168.3.1`:
...
Если в облачном режиме Keenetic не передаёт `X-Forwarded-For`, восстановить адрес клиента нечем: все запросы придут с одного адреса облачного узла. Тогда per-client лимит на контуре непроверяем, и это ограничение обязано быть записано рядом с baseline `#20`, а не обнаружено при разборе его цифр.
