...
1. Запушить релизный коммит в `beta-ddd`. `dotnet` и `security` стартуют сами — записать их run ID.
2. Запустить `observability`, `performance` и `postman` вручную (`Actions` → workflow → `Run workflow`) на той же ветке. Убедиться, что в запуске выбран ровно тот коммит, который станет головой тега.
3. Поставить тег `v1.0.0-rc.N` на этот коммит. `dotnet` и `security` отработают ещё раз, уже от тега.
4. Приложить к GitHub Release все run ID вместе с SBOM и digest образа.

Проверка перед объявлением RC: пять run ID, у каждого `head_sha` совпадает с коммитом, на который указывает тег. Если для workflow ID нет — он не запускался, и это не эквивалент успешного прогона.
...
3. Внести run ID в тело Release (шаблон `EVIDENCE.md` создаётся с пустыми полями намеренно — их нельзя заполнить автоматически, а пустая строка честнее выдуманной).
