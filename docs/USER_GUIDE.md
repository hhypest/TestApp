...
### 4.3 ⚠ Откуда взять текст вопросов и вариантов

**Текущее ограничение реализации:** выделенного endpoint'а «показать студенту вопросы для прохождения» (student-safe presentation DTO без `isCorrect`) в API пока нет — это отдельная **Planned** задача (`ATT-010`, `ATT-011`, `UX-001` в [FEATURE_PLAN.md](FEATURE_PLAN.md), следующая по приоритету после текущего relesae-цикла). Сегодня `questionId`/`optionId` должны быть заранее известны клиенту — на практике их передаёт `test-author`/`test-admin`, создававший тест (или получает через `tests:write`-scoped `GET /api/v1/tests/{testId}/editor`, который студенту недоступен). Планируйте frontend/интеграцию с расчётом на появление presentation API — на сегодняшний день student-flow REST API рассчитан на клиента, который уже знает содержимое revision.
