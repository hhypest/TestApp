...
7. внешняя проба через ingress отказывает **3 пробы подряд** — контур недоступен клиенту, даже если приложение внутри готово. Отдельное правило от пункта 1: они отвечают на разные вопросы и указывают на разные причины (см. ADR-034).
...
8. у внешней пробы нет ни одной цели **10 минут**: пока это так, page-правило 7 не может сработать в принципе — цель генерируется при подъёме контура и могла не появиться;
9. пробер готовности недоступен **5 минут**: пока это так, page-правило 1 не может сработать в принципе — готовность не наблюдается, а не «в порядке». Молчание неотличимо от благополучия, поэтому слепота сама по себе является поводом для тикета.
...
- выражения правил алертов реализованы для каждого правила §4 (8 page + 8 warning: `deploy/prometheus/alerts.yml` для общих, `deploy/staging/alerts-ingress.yml` для правил внешней пробы, существующей только на контуре); единственное невыраженное — warning-правило 7 о конфликтах параллельного доступа: соответствующей метрики нет, и приблизить её нечем. В CI проверяются все, кроме правил внешней пробы: ingress поднимается только на контуре;
- дашборд реализован и проверяется в CI на соответствие минимуму §6 (`deploy/grafana/dashboards/testapp-overview.json`);
- активный пробер готовности: `blackbox_exporter` спрашивает `/health/ready` двумя пробами — внутренней («приложение готово») и внешней через ingress («клиент до него дойдёт»); page-правила 1 и 7 §4 реализованы, внутренняя проверяется в CI на живом стеке (ADR-034);
