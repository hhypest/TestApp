...
**Статус заморозки: черновая.** Снимок и diff-тест введены до того, как фронтенд роли `test-admin` прошёл happy path всех трёх ролей. До этого момента изменения контракта допустимы — они видимы и осознанны, но не запрещены. Окончательная заморозка объявляется отдельно, после чего ломающие изменения возможны только через v2.

**Две legacy-возможности остаются в v1** и не удаляются перед заморозкой: rewrite `/api/*` (ADR-019) и приём `idempotencyKey` в теле запроса (ADR-020). Обе имеют управляемый lifecycle и объявленный exit criteria; удаление до появления telemetry о клиентах сломало бы работающих потребителей ради косметики контракта. Дата удаления назначается вместе с окончательной заморозкой.
