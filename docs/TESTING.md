...
4. unit-проверки release/restore tooling, включая запрет плавающих Action/container references;
...
12. создание логической резервной копии, проверка изолированного восстановления и
    бизнес-маркера, публикация fail-closed JSON evidence;
...
`postgresql-restore-verify.test.mjs` подменяет только Docker boundary и проверяет сам
операционный контракт: evidence появляется после успешной проверки, остаётся прежним при
...
- источник данных для правил готовности — `PrometheusConfigTests`: job внутренней пробы обязан существовать в обоих конфигах, job внешней — только в конфиге контура (ingress есть только там), а модуль пробы — в конфиге экспортера. Список отличий контура от локального стенда закрытый: разрешены ровно блок `alerting` и внешняя проба, посторонний scrape job роняет тест — подтверждено красным. Отказ здесь тихий: правило загрузится и не пожалуется, но при отсутствии серии `probe_success` выражение `== 0` не даёт результата, то есть удаление job выглядит как «всё хорошо».
- окончательная заморозка контракта v1 — `OpenApiFrozenSurfaceTests` (issue #22, ADR-035): живой документ сверяется с `docs/openapi/v1-frozen-surface.json` на включение, а не на совпадение. Дополнения проходят молча — §1 их разрешает; исчезновение или переименование эндпоинта, кода ответа, поля тела ответа или свойства DTO падает с перечислением всего утраченного. Поля ответов заморожены полными путями, потому что тела ответов встроены в операции, а не вынесены в `components`. Подтверждён красным трижды: удалением строки контракта операции, ужесточением поля запроса и правкой самого файла поверхности.
- задание миграций — `scripts/verify-migration-job.sh` в workflow `dotnet` (issue #21 пункт 4): применение baseline на чистой базе production-образом со сверкой числа записей в `__EFMigrationsHistory`, идемпотентность повторного прогона по отпечатку схемы и отказ автоприменения на старте вне Development. Проверяется поведение процесса, а не строка в исходнике. Отпечаток снимается с отфильтрованными `\restrict`/`\unrestrict`: PostgreSQL 18 вставляет в них случайный nonce, и побайтовое сравнение дампов дало бы мигающую проверку. Подтверждён красным дважды — лишней миграцией в репозитории и заданием, меняющим схему при повторном прогоне.
