...
- **Релизный workflow** `.github/workflows/release.yml` на тег `v*`: сборка образа, публикация в GHCR по immutable digest, CycloneDX SBOM и GitHub Release со списком доказательств и замороженным контрактом v1. Сверяет тег с `VersionPrefix` и падает при расхождении. Гейтом качества намеренно не является — гейт это пять существующих пайплайнов на том же коммите, их run ID вносятся в тело Release вручную (`docs/OPERATIONS.md` §16.1–16.2).
